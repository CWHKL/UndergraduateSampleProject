...
public class Nodes {
    ...
    public void logList() {
        Node tmp = head;
        while (tmp != null) {
            log+=("num:" + tmp.number + "  ID:" + tmp.myID + "  states:" + tmp.status + "  data:" + tmp.data + "  NextNode:" + tmp.next.number + "  PreviousNode:" + tmp.previous.number + "  Terminate:" + tmp.terminate + "  Leader in momery:" + tmp.leaderID+"*");
            tmp = tmp.listnext;
        }
    }
    ...
    public void chooseLeaderBiggestTask1() 
    {
        ...
        counter++;
        log+=("Parallel execute " + counter + " finish"+"*");
    ...
    public void NodeActTask1Send() {
        ...
        send = false;
        log+=("Node:<num:" + number + " ID:" + myID + "> pass " + numIn + " to next node <num:" + next.number + " ID:" + next.myID + ">"+"*");
    ...
    public void NodeActTask2_read() {
        ...
        for (NodePackage readTmp : this.PackageRecievedCounterClockwise) {
            log+=("Node:" + this.number + " read<" + readTmp.ID + "," + readTmp.hopCount + "," + readTmp.status + ">"+"*");
    ...
    public void NodeActTask2_send() {
        ...
        for (NodePackage sendTmp : this.PackagePrepareToSendCounterClockwise){
            sendTmp.hopCount--;
            log+=("Node:" + this.number + " sends<" + sendTmp.ID + "," + sendTmp.hopCount + "," + sendTmp.status + "> to Node:" + previous.number+"*");
            sendPackageCounterClockWise(sendTmp);
        }
        //send clockwise
        for (NodePackage sendTmp2 : this.PackagePrepareToSendClockwise){
            sendTmp2.hopCount--;
            log+=("Node:" + this.number + " sends<" + sendTmp2.ID + "," + sendTmp2.hopCount + "," + sendTmp2.status + "> to Node:" + next.number+"*");
    ...
    private void becomeLeader() {
        ...
        this.PackagePrepareToSendClockwise.clear();
        this.PackagePrepareToSendCounterClockwise.clear();
        log+=("Node:<num:" + number + " ID:" + myID + "> chosen as the leader"+"*");
        Rounds=counter+1;
    }
    ...
    public void sendTerminate() {
        if(this.terminateTosend){
            ...
            this.terminate=true;
            log+=("Node:<num:" + number + " ID:" + myID + "> told Node:<num:" + next.number + " ID:" + next.myID + "> to terminate"+"*");
        }
    }
    }
}
